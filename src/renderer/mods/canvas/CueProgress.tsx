...
const Marker = styled.div`
  ...
  border-bottom: 2px solid #222;
  border-left: 0.5em solid ${(props: any) => props.color};
`
...
const MarkerData = ({ data }) => {
  ...
  return (
    <MarkerDataWrap>
      ...
      {todaysEntry ? (
        <div style={{ color: 'green' }}>+{moment.utc(todaysEntry.duration).format('H:mm:ss')}</div>
      ) : null}
    </MarkerDataWrap>
  )
}
    ...
    <div>
      ...
      <Wrap>
        {markers.map((marker, i) => {
          return (
            <Marker key={marker.name + i} color={marker.color}>
              {marker.name}
