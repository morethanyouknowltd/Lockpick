...
const Message = styled.div`
  ...
  animation: flashIn 4s linear 1, slideIn 0.3s linear 1;
  ...
  while (emotes.length) {
    ...
    parts.push(
      <img
        ...
        key={elementI++}
        src={`https://static-cdn.jtvnw.net/emoticons/v1/${thisEmote.id}/2.0`}
      />
    )
...
export const TwitchChat = ({ messages }) => {
  return (
    <Wrap>
      <PanelTitle>
        <FontAwesomeIcon icon={faComments} style={{ marginRight: '.5em' }} /> twitch chat
      </PanelTitle>
      <Messages>
        {messages.slice(-50).map(message => {
          ...
          return (
            <Message key={message.messageId}>
              <Date>[{date.format('HH:mm')}]</Date>{' '}
              <UserName style={{ color: message.colorRaw ? message.colorRaw : '#777' }}>
                {message.displayName}
              </UserName>
