...
const MarkerItem = styled.div`
  ...
  border-left: 5px solid
    ${props => (props.disabled ? 'rgba(255,255,255,0.04)' : props.marker.color)};
  padding: 0.4em 0.6em;
  cursor: pointer;
  &:hover {
    background: #222;
  ...
  user-select: none;
  color: ${(props: any) =>
    props.active ? 'white' : props.disabled ? 'rgba(255, 255, 255, .04)' : '#888'};
  background: #222;
  > :nth-child(1) {
    display: flex;
    height: 100%;
...
export const TransportNavPopup = ({ cueMarkers, position, sendData, popup }) => {
  ...
  let shortcutIndex = -1
  return (
    <TransportWrap positionPercentX={position / rangeAmount} coverUpToX={coverUpToX}>
      {cueMarkers.map((marker, i, arr) => {
        ...
        const props = {
          ...
          style: {
            ...
            width: nextMarker
              ? `${(((nextMarker.position - marker.position) / rangeAmount) * 100).toFixed(2)}%`
              : '',
            right: nextMarker ? '' : 0,
          },
