...
const TrackIcon = ({ track, selected, allowDeleting, allowAdding, onClick }) => {
  ...
  return (
    <TrackIconWrap selected={selected} onClick={onClick}>
...
export const TrackResult = React.memo(
  ({ result, selected, onConfirmed, onShouldSelect, options }: TrackResultProps) => {
    ...
    return (
      <Result
        inCue={isInCue}
        ref={wrapRef}
        id={selected ? 'selectedtrack' : ''}
        key={track.id}
        selected={selected}
        onDoubleClick={() => onConfirmed(result)}
        onMouseDown={() => onShouldSelect(result)}>
        <Color color={track.color} />
        <TrackIcon
          selected={selected}
          track={track}
          onClick={onIconClick}
          allowAdding={mouseOverWithAlt && !isInCue}
          allowDeleting={mouseOverWithAlt && isInCue}
        />
