...
const TrackSearchImpl = (tracks: BitwigTrack[], options: TrackSearchOptions) => {
  ...
  const similarity = (track: BitwigTrack, query: string): number => {
    ...
    if (indexOfMatch >= 0) {
      ...
      return (
        1 -
        indexOfMatch / filtered.length +
        (inCue ? 1 : 0) +
        (recentI >= 0 ? (recentTracks.length - recentI) * 0.5 : 0)
      )
    }
    return 0
  }
...
export class TrackSearchView extends React.Component<SearchProps> {
  ...
  onKey = (key: string) => {
    ...
    if (key === 'ArrowUp') {
      ...
      const newSelect =
        this.state.results[Math.min(this.state.results.length - 1, selectedIndex + 1)]
      this.onShouldSelect(newSelect)
      this.waitingForScroll = true
    }
  }
  ...
  recreateSearcher(tracks?: BitwigTrack[], options = this.props.options, recalculate = true) {
    ...
    this.recentSearch = TrackSearchImpl(
      recentTracks.map(id => getTrackById(id)).filter(t => !!t),
      options
    )
    if (recalculate) {
  ...
  onConfirmed = (res: SearchResult) => {
    ...
    recentTracks = [track.id].concat(
      recentTracks.slice(0, recentCount).filter(id => id !== track.id)
    )
    saveRecent10()
  ...
  renderResults() {
    return (
      <>
        <RecentsHeader large>
          {this.props.query.length === 0 ? 'Recent Tracks' : 'Results'}
        </RecentsHeader>
        {this.state.results
          .slice(0, this.state.resultsLimit)
          .map(res => this.mapToTrackResult(res))}
      </>
    )
  }
  ...
  render() {
    return this.state.results.length > 0 ? (
      <FlexContainer style={{ fontSize: '.9rem' }}>
