...
const SidebarItem = ({ children, filterId, onClick, ...rest }) => {
  return (
    <SidebarItemWrap onClick={onClick} {...rest}>
      {children}
    </SidebarItemWrap>
  )
}
...
const SidebarSection = ({ title, children }) => {
  return (
    <SidebarSectionWrap>
      {title ? (
        <div style={{ color: 'rgb(131 131 131)', marginBottom: '.8em' }}>{title}</div>
      ) : null}
      {children}
    </SidebarSectionWrap>
  )
      ...
      {mods.map(mod => {
        ...
        return (
          <HasContextMenu
            ...
            <SidebarItem
              key={mod.key}
              focused={(currentMod?.id ?? null) === mod.id}
              onClick={() => setCurrentMod(mod)}
              filterId={mod.key}
              ...
              <Flex justifyContent={'space-between'}>
                <div
                  ...
                  <span style={{ marginRight: '.8em', width: '.6em' }}>
                    <Indicator on={enabled ? 'true' : ''} />
                  </span>
                  <span>
                    {mod.name || mod.key}
                    {mod.error ? (
                      <Warning title={`There was an error loading this mod (click for details)`} />
                    ) : null}
                  </span>
                </div>
      ...
      <div style={{ padding: '1.5em', paddingTop: 0 }}>
        ...
        {Object.keys(modsByCategory)
          ...
          .map(category => {
            ...
            return (
              <SidebarSection title={humanise(category)} key={category}>
                <SidebarModList
                  currentMod={currentMod}
                  setCurrentMod={setCurrentMod}
                  mods={modsForThisCategory}
                />
              </SidebarSection>
            )
          })}
