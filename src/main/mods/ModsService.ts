...
export class ModsService extends BESService {
    ...
    isActive() {
        return Bitwig.isActiveApplication() || (process.env.NODE_ENV === 'dev' ? Bitwig.isActiveApplication("Electron") : Bitwig.isActiveApplication(APP_NAME))
    }
    ...
    async makeApi(mod) {
        ...
        async function createOrUpdateTrack(track: string, project: string, data: any) {
            ...
            if (existingTrack) {
                logger.info(`Updating track (${existingTrack.name} (id: ${existingTrack.id})) with data: `, data)
        ...
        const api = {
            ...
            Db: {
                ...
                setData: async (data) => {
                    const existingSetting = (await this.settingsService.getSettingValueOrNull(`mod/${mod.id}`)) || {}
    ...
    async getModsWithInfo({category, inMenu, modId} = {} as any) : Promise<(ModInfo & {key: string, value: any})[]> {
        const db = await getDb()
        ...
        return Object.keys(this.latestFoundModsMap).filter(key => {
    ...
    async activate() {
        ...
        addAPIMethod('api/mod', async ({ id }) => {
            const mod = 
            (await this.getModsWithInfo({ modId: id }))[0] as any
            const db = await getDb()
            const settings = db.getRepository(Setting) 
    ...
    async getModsFolderPaths() : Promise<string[]> {
        ...
        return [
            ...
            ...(exists ? [path.join((await this.settingsService.lockpickLibraryLocation())!, 'Mods')] : [])
        ]
    }
    ...
    async copyControllerScript() {
        ...
        try {
            ...
            for (const file of [...(await fs.readdir(controllerSrcFolder)), 'mods.js']) {
                const src = (await fs.readFile(path.join(file === 'mods.js' ? buildModsPath : controllerSrcFolder, file))).toString().replace(
        ...
        for (const modsFolder of paths) {
            ...
            for (const filePath of files) {
                ...
                try { 
                    ...
                    const osMatches = os === '' || os.split(',').some(os => {
