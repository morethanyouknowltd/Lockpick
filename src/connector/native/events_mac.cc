...
CGEventRef eventtap_callback(CGEventTapProxy proxy, CGEventType type, CGEventRef event, void *refcon)
{
    ...
    auto bitwigProcessIds = getAllPidsFromProcesses();
    // If the target application is not bitwig, don't process the event at all
    if (std::find(bitwigProcessIds.begin(), bitwigProcessIds.end(), targetPID) == bitwigProcessIds.end())
    {
        return event;
    }
...
Napi::Value on(const Napi::CallbackInfo &info)
{
    ...
    auto ourInfo = addEventListener(EventListenerSpec({eventType,
                                                       nullptr,
                                                       &cb,
                                                       env}));
